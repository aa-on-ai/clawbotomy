...
- **Terminal Phase 9 experiment:** PR #15 is frozen at `c52d370`. Its control remains valid offline findings evidence and must not be retried. One approved `completion-evidence-gate` treatment attempt ended in infrastructure failure without a complete bundle, so it supports no comparison, remedy claim, or result-supported fix. The approval is consumed and no additional provider run is authorized.
- **Local Live Bench:** a separate deterministic, zero-provider personality-trajectory slice is implemented on a local review branch. It is not shipped, deployed, publicly navigated, or real-agent evidence; real-agent mode remains separately gated.
