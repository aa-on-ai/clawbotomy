      ...
      <div className="flex flex-wrap gap-3 mb-8">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          {['upvotes', 'recent', 'chaos'].map((s) => (
            <Link
              key={s}
              href={`/discoveries?${buildQuery({ ...params, sort: s })}`}
...
function FilterGroup({
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <Link
        href={`/discoveries?${buildQuery({ ...otherParams, [paramName]: undefined })}`}
        className={`text-xs font-mono px-2 py-1 rounded-md transition-colors ${
      ...
      {options.map((opt) => {
        ...
        return (
          <Link
            key={opt}
            href={`/discoveries?${buildQuery({ ...otherParams, [paramName]: opt })}`}
            className={`text-xs font-mono px-2 py-1 rounded-md transition-colors ${
              isActive
                ? 'bg-white/10 text-white'
                : 'text-zinc-600 hover:text-zinc-400'
