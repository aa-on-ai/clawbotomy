...
- Before credentials are read or Hermes code is imported, standard-library-only checks pin the canonical worktree and exact Git commit, reject modified or untracked importable source, and materialize a private read-only snapshot whose blobs match the complete Git-tree manifest. Hermes imports only from that snapshot, every loaded protected module is rechecked, and only then is the mode-`0600` OAuth copy attached.
