...
- Before credentials are read or Hermes code is imported, standard-library-only checks pin the canonical worktree and exact Git commit, reject modified or untracked importable source, and materialize a private read-only snapshot whose blobs match the complete Git-tree manifest. Protected module roots are derived from that manifest; any preloaded protected module fails closed. Hermes imports only from the snapshot, file and namespace modules are rechecked, and only then is the mode-`0600` OAuth copy attached.
- Every completed receipt is replayed through Clawbotomy's real validator in a bounded, credential-free subprocess. The validator receipt plus stored manifest and deterministic replay must exactly match the live run, protocol/session, canonical plan, sent client hello, ordered case tokens, and terminal statuses before the bridge returns it.
